...
<body>
	...
	<form action="ConfiguracionLCDAndroid?a=${a}" method="post" id="formconfigcaja">
		<div class="section">
			<div class="container">
				<div class="row">
					<div class="col-md-1">
			...
			<div class="container">
				<div class="row">
					<div class="col-md-12 scroll" style="border-radius: 25px; border: 2px solid white; padding: 20px; height: 65%; overflow: auto;"	id="disponibles">
						<div class="row h4">
							<div class="col-md-4 h1">LCDs</div>
							<div class="col-md-8 h1 text-center">ZONAS</div>
						</div>
						<c:forEach items="${lcds}" var="l">
							<div class="row">
								<div class="col-md-4 h2">${l.disp_id}</div>
								<div class="col-md-8 h4">
									<div class="checkbox-toolbar" >
										<input type="checkbox" id="radio1${l.disp_id}"	name="LCD_${l.disp_id}" value="1"	<c:if test="${l.z1 eq 1}"> checked="checked"</c:if>>
											<label for="radio1${l.disp_id}">ZONA 1</label>
										<input type="checkbox" id="radio2${l.disp_id}" name="LCD_${l.disp_id}" value="2"	<c:if test="${l.z2 eq 2}"> checked="checked"</c:if>>
											<label for="radio2${l.disp_id}">ZONA 2</label>
										<input type="checkbox" id="radio3${l.disp_id}" name="LCD_${l.disp_id}" value="3"	<c:if test="${l.z3 eq 3}"> checked="checked"</c:if>>
											<label for="radio3${l.disp_id}">ZONA 3</label>
										<input type="checkbox" id="radio4${l.disp_id}" name="LCD_${l.disp_id}" value="4"	<c:if test="${l.z4 eq 4}"> checked="checked"</c:if>>
											<label for="radio4${l.disp_id}">ZONA 4</label>
										<input type="checkbox" id="radioNA${l.disp_id}" name="LCD_${l.disp_id}" value="100"	<c:if test="${l.sz eq 100}"> checked="checked"</c:if>>
